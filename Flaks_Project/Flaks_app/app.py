...
from model.currentWeeks import getCurrentWeeks
from model.routingFilter import routingFilter
...
def index():
	...
	return render_template('index.html', wip_modif = wip_modif,update_wip = wipRoutings.query.filter_by(STATUS = 'In Progress').all(),weeks = availableWeeks)
...
def booking():
	...
	d = WIP.query.filter_by(WIP_ENTITY_NAME = job_no).first()
	print(d)
	check_if_exists = wipRoutings.query.filter_by(WIP_ENTITY_NAME = 6546, STATUS = 'In Progress').first()
	print(f'Sub check is: {check_if_exists.PART_NUMBER}')
	...
	if job_rec == None:
		...
		#data = WIP.query.filter_by(WIP_ENTITY_NAME = job_no).all()
